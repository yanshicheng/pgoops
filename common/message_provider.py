...
from common.ding_talk_pub import DingTalkPub
from common.lark_pub import LarkPub
...
class MessageProvider:
    ...
    def at_to_user(self):
        raise NotImplementedError(
            f"{self.__class__.__name__}, There is no implementation  send_to_group  methods"
        )
...
class DingTokProvider(MessageProvider):
    def __init__(self, instance):
        ...
        self.provider = DingTalkPub(**ConfigDispose.get_alert_provider_dingtalk())

    def send_text_msg(self):
        text_message = self.test_message()
        self.provider.send_text_msg(
            content=text_message,
            at_mobiles=self.get_user_list(),
            is_at_all=self.is_at_all,
        )

    def send_markdown_msg(self):
        markdown_message = self.markdown_message()
        self.provider.send_markdown_msg(
...
class EmailProvider(MessageProvider):
    ...
    def send_html_msg(self):
        ...
        self.provider.send_html_msg(
            content=html_message, to_user=self.get_user_list(), subject=self.title
        )
...
class WeChatProvider(MessageProvider):
    ...
    def send_text_msg(self):
        ...
        self.provider.send_text_msg(
            content=text_message,
            mentioned_mobile_list=self.get_user_list(),
        )

    def send_markdown_msg(self):
        markdown_message = self.markdown_message()
        self.provider.send_markdown_msg(
            content=markdown_message, mentioned_mobile_list=self.get_user_list()
        )
...
class LarkProvider(MessageProvider):
    def __init__(self, instance):
        super(LarkProvider, self).__init__(instance)
        self.provider = LarkPub(**ConfigDispose.get_alert_provider_lark())
