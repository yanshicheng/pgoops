...
class TaskModelViewSet(StandardModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        repository_obj = Repository.objects.filter(
        ...
        instance.save()
        ws_url = f'ws://{request.META["HTTP_HOST"]}/ws/iac/task/{instance.id}/'
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if instance.from_periodic:
            return api_error_response(
                f"定时任务执行记录不可删除，如需删除请先删除定时任务:{instance.from_periodic.name}"
            )
