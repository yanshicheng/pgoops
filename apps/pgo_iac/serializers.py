...
class BaseTaskModelMixinSerializer(StandardModelSerializer):
    ...
    def to_representation(self, instance):
        representation = super(BaseTaskModelMixinSerializer, self).to_representation(
            instance
        )
        representation["repository_name"] = instance.repository.project_name
        representation["playbook"] = (
            instance.playbook if instance.playbook else instance.repository.main
        )
        try:
...
class TaskInfoSerializer(TaskSerializer):
    ...
    def to_representation(self, instance):
        ...
        representation["task_stats"] = TaskStatsSerializer(
            TaskStats.objects.filter(task_record=instance.id), many=True
        ).data
        return representation
...
class TaskPeriodicSerializer(BaseTaskModelMixinSerializer):
    ...
    def update(self, instance, validated_data):
        interval = self._update_child(
            IntervalSchedule, validated_data.pop("interval", None)
        )
        crontab = self._update_child(
            CrontabSchedule, validated_data.pop("crontab", None)
        )
        if interval:
