...
class PrepareHandler:
    def __init__(self, code_package):
        ...
        # 包解压路径
        self.abs_package_path = os.path.join(
            MEDIA_ROOT, self.work_dir, self.package_name
        )
        # django 机制写入
    ...
    def _package_verify(self, layout, mode="r", method="create"):
        try:
            ...
            if not site or not hosts:
                raise Exception(
                    "必须包含入口文件:./package_name/project/deploy.yaml， hosts文件: ./package_name/project/hosts"
                )
    ...
    def _tmp_save(self):
        default_storage.save(
            self.tmp_package_name, ContentFile(self.code_package.read())
        )
...
class EventHandler(BaseEventHandler):
    def _save_event(self, data: dict, state: EventState):
        ...
        TaskEvent.objects.create(
            ...
            changed=data["res"].get("changed")
            if "changed" in data["res"]
            else data["res"]["_ansible_no_log"],
            detail=data["res"],
        )
    ...
    def playbook_on_stats(self, data: dict):
        ...
        for k, v in data.items():
            ...
            for host, count in v.items():
                stats = stats_map.get(
                    host, TaskStats(task_record=self.Task, host=host, ip=resolv(host))
                )
...
class TaskRunner:
    ...
    def execute(
        ...
        try:
            ansible_runner.run(
                private_data_dir=self.work_dir,
