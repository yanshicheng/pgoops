...
def show_genres(request):
    return render(
        request, "service_tree/genres.html", {"genres": ServiceTree.objects.all()}
    )


class ServiceTreeModelViewSet(BulkCreateModelMixin, StandardModelViewSet):
    """服务树"""

    queryset = ServiceTree.objects.filter()
    ...
    def list(self, request, *args, **kwargs):
        ...
        return api_ok_response(data=data)
    ...
    def opera_permission_member(self, request, pk=None):
        ...
        return api_ok_response(data=ret)
    ...
    #     }
    #     return api_ok_response(code=0, data=response, message=None)
    ...
    def tag(self, request, pk=None, *args, **kwargs):
        ...
        for idx, n in enumerate(_nodes):
            kvs = [
                {"key": info.key, "value": info.value, "id": info.pk}
        ...
        return api_ok_response(data=response)
    ...
    def server_filter(self, qs, query):
        ...
        if dft_v:
            qs = qs.filter(Q(hostname__contains=dft_v) | Q(private_ip__contains=dft_v))
        else:
            # 精准搜索
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        if not ins.is_leaf_node():
            return api_error_response(message="不允许跨层级删除.")
        return super(ServiceTreeModelViewSet, self).destroy(request, *args, **kwargs)


class NodeOperaPermissionModelViewSet(StandardModelViewSet):
    """服务树 关联节点操作权限"""

    queryset = NodeLinkOperaPermission.objects.filter()
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return api_error_response(e.args)
...
# class NodeLinkServerModelViewSet(StandardModelViewSet):
#     """
#     服务树 关联服务器
#     """
...
#                 return api_ok_response(code=-1, data=None, message="app_key not found.")
#
#             request_data = {
#                 "node": node.node_id,
...
#         if serializer.is_valid():
#             self.perform_create(serializer)
#             return api_ok_response(code=0, data=serializer.data, message=None)
#         else:
#             try:
#                 instance = self.queryset.get(node_id=serializer.data['node'])
#             except NodeLinkServer.DoesNotExist:
#                 return api_ok_response(code=-1, data=None, message="not found.")
#
#             partial = kwargs.pop('partial', True)
...
#         except NodeLinkServer.DoesNotExist:
#             return api_ok_response(code=-1, data=None, message=f"node_id {node_id} not found.")
#
#         instances = CMDBBase.objects.filter(pk__in=server_ids)
#         node_link_server.cmdbs.remove(*[x.pk for x in instances])
#         return api_ok_response(code=0, data=None, message=None)


class ParentNodeInfoApiView(StandardApiView):
    """
    查询节点上所有的父节点
    ...
    def get(self, request, pk, *args, **kwargs):
        try:
            ...
            return api_error_response(f"pk {pk} not found.")
