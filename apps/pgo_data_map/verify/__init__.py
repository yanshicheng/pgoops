...
class Int(BaseType):
    def stringify(self, value):
        ...
        if max and val != 0 and val > max:
            raise ValueError(
                f"field {self.field} Too big value to {value} Limited to {max}"
            )
        min = self.min
        if max and val != 0 and val < min:
            raise ValueError(
                f"field {self.field} Too small value is {value} Limited to {min}"
            )
        return val
    ...
    def stringify(self, value):
        ...
        if val:
            ...
            if prefix and not str(val).startswith(prefix):
                raise ValueError(
                    f"field {self.field} is {value} not startswith {prefix}"
                )
            return str(ipaddress.ip_address(value))
        return val
...
class Str(BaseType):
    def stringify(self, value):
        ...
        if len_num and len(value) > len_num:
            raise ValueError(
                f"field {self.field} is {value} Out of length limit, should be less than  {len_num}"
            )
        return str(value)
...
class Choice(BaseType):
    def stringify(self, value):
        if len(self.choice) < value:
            raise ValueError(
                f"field {self.field} Choice value:({value}) Out of maximum index"
            )
        return value
