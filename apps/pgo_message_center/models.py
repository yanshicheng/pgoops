...
__all__ = ["Provider", "Group", "Level", "History"]
...
class Provider(StandardModelMixin):
    ...
    icon = models.ImageField(
        upload_to="message_center/icon/%Y/%m/%d/",
    ...
    provider_class = models.CharField(
        max_length=128, validators=[message_provider_class]
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if not hasattr(clazz, self.get_method_display()):
            raise ValidationError(
                f"{self.name} not {self.get_method_display()} method！"
            )
...
class History(StandardModelMixin):
    ...
    instance = models.CharField(
        max_length=1024, blank=True, null=True, verbose_name="实例地址"
    )
    labels = models.JSONField(verbose_name="扩展信息", blank=True, null=True)
    summary = models.CharField(
        max_length=1024, verbose_name="概要信息", blank=True, null=True
    )
    ...
    duration = models.CharField(
        verbose_name="持续时间", max_length=64, blank=True, null=True
    )
    ...
    def get_markdown_message(self, sign="\n"):
        msg = (
            f'### **{History._meta.get_field("app_name").verbose_name}:** {self.app_name} ({self.get_type_display()}) {sign}'
